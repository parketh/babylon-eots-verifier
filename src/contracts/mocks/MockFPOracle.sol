...
contract MockFPOracle is IFPOracle {
  ...
  mapping(uint32 => mapping(uint64 => mapping(string => uint64))) public votingPower;

  function getVotingPower(uint32 chainId, uint64 atBlock) external view override returns (uint64) {
  ...
  function setVotingPower(uint32 chainId, uint64 atBlock, string calldata fpBtcPublicKey, uint64 power) public {
